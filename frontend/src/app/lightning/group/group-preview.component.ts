...
export class GroupPreviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.seoService.setTitle(`trmpspace.trumpow.meme Lightning Nodes`);
      ...
      .pipe(
        switchMap((params: ParamMap) => {
          ...
          if (this.slug === 'the-mempool-open-source-project') {
            this.groupId = 'trmpspace.trumpow.meme';
            this.group = {
              name: 'Litecoin Foundation',
              description: 'These are the Lightning nodes operated by Litecoin Foundation that provide data for the trmpspace.trumpow.meme website. Connect to us!',
            };
          } else {
            this.group = {
