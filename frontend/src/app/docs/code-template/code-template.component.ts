...
init();`;
  ...
  wrapCommonJS(code: any) {
    ...
    if (code.codeTemplate) {
      ...
      const importText = `<script src="https://trmpspace.trumpow.meme/mempool.js"></script>`;
...
yarn add @mempool/mempool.js`;
  ...
  wrapPythonTemplate(code: any) {
    return ( ( this.network === 'testnet' ) ? ( code.codeTemplate.python.replace( 'wss://trmpspace.trumpow.meme/api/v1/ws', 'wss://trmpspace.trumpow.meme/' + this.network + '/api/v1/ws' ) ) : code.codeTemplate.python );
  }
